...
def manage_keys(passphrase,selcmd): #["gen-key","import","export","export-secret","delete-pub-key","delete-priv-key"]
	...
	if selcmd =="import":
		...
		if selcmd in ["export","delete-pub-key"]:
			...
			if gpguid=='':
				return
			exp_dir_file=os.path.join('my_files',gpguid.replace('@','').replace('.','')+'.pgpkey')
			...
			if gpguid=='':
				return
			exp_dir_file=os.path.join('my_files','secret_'+gpguid.replace('@','').replace('.','')+'.pgpkey')
...
def encr_msg(str_cont,encr_type,keyorpass,internal_id_str): # fnam should be indexed file ... to not have simmilar names ... save in folder sent ...
	...
	fname=os.path.join('archive','sent','sent_'+internal_id_str+'.txt.pgp')
	...
	if os.path.exists(str_cont): # if file!!  if tmpfile!='':
		...
		fname=os.path.join('archive','sent',headtail[1]+'.pgp')
	...
	if encr_type=='aes256':
		...
		if tmpfile=='':
			tmpfile=createtmpfile(str_cont)
		gpg_tmp="gpg -a -r "+keyorpass+" -o "+fname+" -e "+tmpfile
		str_rep=subprocess.getoutput(gpg_tmp)
...
def decr_msg(msg_cont,gpgpass='',aes256pp=''): #,msg_from
